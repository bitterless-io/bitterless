...
- Extend the existing bounded worksheet preflight to count `sheetData` by local XML name while it
  already streams each worksheet part. Normalize only when the trusted descriptor extension is
  exactly `.xlsx`, with exactly one worksheet and exactly one missing `sheetData`; multi-sheet,
  `.xlsm`, macro-bearing, and multiple-`sheetData` cases remain invalid rather than rewriting a
  complex workbook.
...
- The compatibility Worker receives only the already authorized `ArrayBuffer`; it never receives a
  ...
  is never modified.
- Keep the existing 25 MiB ordinary Office ceiling, enforce the tighter compatibility ceilings,
  terminate the Worker on completion/error/disposal/timeout, and never run normalization for XLSM,
  DOCX, PPTX, invalid ZIPs, encrypted packages, archive-limit failures, arbitrary Viewer failures,
  or a second load failure.
...
- The reported single-empty-sheet reimbursement workbook renders through the normal OOXML Viewer
  after at most one pre-Viewer compatibility normalization; current-file Find remains owned by that
  Viewer.
...
- Replacement or disposal cannot publish a stale normalization result, retain either full buffer,
  or leave a Viewer/Worker alive.
...
Implemented by [Task 088](../plan/tasks/onlypreview-xlsx-compatibility-repair-088.md) and approved
by [independent review 1](../plan/reviews/onlypreview-xlsx-compatibility-repair-088-1.md) with no
blocking P1/P2. Ral retains final live visual acceptance for both reported workflows.
