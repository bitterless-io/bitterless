...
### #5.1 只关一个时不列标签页名(Ral 2026-09-20 裁决)

`terminalLabels` 那一行**只在关多个时渲染**。Ral 2026-09-20:「zellij 标题多余了,直接展示提示
文字」。

理由是它在单数情况下不携带任何信息:你是点了**那一个** tab 的关闭按钮才走到这里的,列表把那个
tab 的名字原样重复一遍(截图里就是孤零零一行 `Zellij`),读起来像个标题,但下面那句正文已经说清
了要结束什么。复数时它仍然渲染 —— 那时它是**唯一**说明「要关的是哪几个终端」的东西,删掉就成了
一次数不清范围的破坏性确认。

正文同时改写成自带主语,把被删掉那行的信息接过来,并说明代价不止于会话本身:
...
「pane 里正在跑的东西也会停」不是修辞:原生关闭走 `KillSession`,
`tests/zellij/zellijNativeIpc.native.test.mjs` 断言 server 与 **shell 进程**都死亡。
