...
The macOS ARM fast-publish shortcut synchronizes source first, installs the checked-out Yarn
lockfile with `--frozen-lockfile`, then runs the existing patch preparation to increment
`version`/`_version` and generate a fresh local `YYMMDDHHmmss` `version_code`. Only then may it start
the signed build. An install or patch failure stops the command before packaging. This prevents a
newly pulled Electron or SQLCipher dependency from being omitted due to stale `node_modules`, and
prevents a new fast-published version from reusing an older release's build identifier.
