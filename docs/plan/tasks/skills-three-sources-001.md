...
Ral approved option A and explicitly requested merging both backend/API repositories from `dev/next` into `main`. `bitterless-private` and `micromeet-bruno` now use the configured `main` branch with the complete `dev/next` history included and retained. The desktop supplemental tests/evidence are committed on `dev/next`; this final handoff also corrects the historical design-stage deployment note. Product runtime code and cloud artifacts are unchanged.
