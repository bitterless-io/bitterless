...
| Shell renderer                         | workspace/listings/index/settings/grouped search batches/result previews/progress/Main presentation | rooted Project tree, right-workspace Global Search, one current-file Find Bar, lazy result preview, 2px rail, 43px Preview toolbar/actions, status, inner bounds             | preload XPC, Arco, Tabler icons                                                  | renderer source/type tests                                                                        |
...
21. Shell initializes and queries a narrow Main XPC proxy. Main validates the attached host, resolves
    ...
    executes/cancels queries. It owns one-active/one-latest execution, emits at most 250 Files plus
    250 Contents rows with text-only grapheme snippets, retains only the latest bounded result-token
    map, and publishes only aggregate
    ...
    Project-bottom rail. During a build/reconcile, manual file activation additionally replaces one
...
22. Entering Global Search captures a stable Current directory anchor and sends a strict relative
    ...
    separate action and closes Global Search.
...
34. Every Vue format component is dynamically selected by the current adapter. `.drawio` selects a
    lazy `DrawioPreview`, whose 20MiB file override comes from the shared adapter size-policy
    dictionary with a 10MiB default. It transfers the exact asset to a disposable bounded Worker;
    only a successful fixed-chunk outer XML plus streaming base64/DEFLATE/percent/UTF-8/XML preflight
    loads the locally pinned official viewer into a component-owned DOM node. The Worker caps
    expanded content at 32MiB and rejects all embedded/external raster/SVG/data/blob image resources,
    image shape/source and image markup before viewer/GPU work. It uses no iframe, editor, remote
    service, Main/preload parser, or phase-one Find capability. Worker/viewer/watchdog teardown
    remains fenced by exact runtime and selection revision.
...
`onlypreview-selected-file-index-priority-034` adds one bounded latest-selected-file lane beside that
candidate. A manually opened eligible file is independently validated and indexed as one complete
in-memory row, so its verified match can arrive while first-build Global Search is still pending;
the incomplete candidate remains unqueryable and the complete terminal result replaces the early
projection after scope completion or atomic promotion. Independent review 3 passed after closing
failure-barrier poisoning, traversal-depth parity, and TS-1 module-boundary findings; owner runtime
verification remains pending.

Tasks 035–037 supersede the earlier Project-filter and merged Project Search presentation without
discarding the completed indexing core. Task 035 makes known specialized adapters authoritative and
then treats any remaining regular file within the separate 8MiB Preview / 1MiB search-body caps as
plain text, including compound or unknown suffixes such as `AGENTS.md.bak`; it intentionally performs
no binary-byte sniff and never executes fallback content. Its focused 46/46, Node typecheck, build,
and diff checks pass; independent review 2 records PASS after review 1's stale-doc blocker was fixed.
Task 036 splits query output into capped
Files and Contents groups and adds latest-request opaque result tokens for 256KiB text heads,
200-child directory listings, bounded verified match contexts, or metadata-only non-text previews.
Its focused 28/28, Node/strict TypeScript, build, and diff checks pass. Independent review 2 records
PASS after closing review 1's stale file identity, failed-replacement revocation, streamed section
cap, and Markdown-adapter findings.
Task 037 removes the Project filter, inserts one initially expanded synthetic root row, and mounts
the Global Search plus lazy bottom preview in the right workspace while native Preview content bounds
are temporarily zero. `Shift+Cmd/Ctrl+F` is the sole global shortcut; selection previews a result,
whereas double click or `Cmd/Ctrl+Enter` explicitly opens/reveals it. Its independent review 1 found
and closed a Find-to-Global hidden-Preview focus trap, opener-restoration drift, persistent collapsed
groups, and dead Project Search presentation. Review 2 records PASS with no P0–P3 finding; focused
and adjacent review tests pass 52/52, with owner live-app verification still pending.
...
text admission gates. Main Preview text uses bounded tolerant UTF-8/BOM UTF-16 reads; Global Search
...
`onlypreview-drawio-readonly-032` is `implemented; owner verification pending` after
[independent review 3](../reviews/onlypreview-drawio-readonly-032-3.md) recorded **PASS** with no
remaining correctness, security, performance, or device-freeze blocker. It provides a no-iframe
locally pinned viewer, a 20MiB adapter override over the shared 10MiB default, bounded streaming
...
| large directories freeze UI                                                   | cooperative background-preload metadata traversal, time-sliced batches, `backgroundThrottling: false`, Global Search fixed/ordered body-index exclusions, cancellation/generation fences                                           |
...
| Monaco or raw content intercepts current-file find shortcut                   | bind exact non-repeat plain `Cmd/Ctrl+F` to the Shell Find Bar; route exact `Shift+Cmd/Ctrl+F` to Global Search across Shell/Vue/Chrome, and never open Monaco's widget                                                            |
...
| A manually opened file waits behind the first full index                      | relay one exact relative path to a hidden-preload latest-only priority lane; reuse normal admission/identity guards, publish only a complete one-file match, then replace it with the authoritative terminal result                  |
...
| Unknown or compound suffix is wrongly rejected as non-text                    | known specialized adapters win first; every remaining regular file within the correct surface cap uses non-executable tolerant plain-text fallback                                                                              |
