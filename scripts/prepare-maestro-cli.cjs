...
const STAGE_DIR = path.join(WORKSPACE_DIR, 'build', 'maestro-tools')
...
const main = () => {
  ...
  if (args.help) {
    console.log(`node scripts/prepare-maestro-cli.cjs <${Object.keys(PLATFORMS).join('|')}> [--skip-build]`)
    return
  }
...
try {
  ...
  console.error(`[prepare-maestro-cli] ${(err && err.message) || String(err)}`)
  process.exit(1)
}
