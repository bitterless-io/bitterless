...
for (const file of [
  ...
  assert(existsSync(join(root, file)), `networking layer file should exist: ${file}`)
}

const electronViteConfig = readFileSync(join(projectRoot, 'electron.vite.config.ts'), 'utf8')
const rendererEndpointClient = readFileSync(join(root, 'renderer/common/networking/clients/endpoint.client.ts'), 'utf8')
const rendererEndpointApi = readFileSync(join(root, 'renderer/common/networking/api/coachEndpoint.api.ts'), 'utf8')
const mainRelayClient = readFileSync(join(root, 'main/networking/clients/relay.client.ts'), 'utf8')
...
assert(rendererEndpointClient.includes('@cowork-shared/networking/coachEndpoint'), 'renderer networking client should share endpoint resolution code')
...
const { buildAiCrmsPiProviderConfig } = loadTsModule('@cowork-main/networking/api/aiCrmsRelay.api')
...
} = loadTsModule('@cowork-shared/networking/coachEndpoint')
...
const { buildAiCrmsPiProviderConfig: buildWithBuildConstants } = loadTsModule('@cowork-main/networking/api/aiCrmsRelay.api')
