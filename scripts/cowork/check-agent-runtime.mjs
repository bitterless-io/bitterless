...
const aiCrmsRuntimeCheck = readFileSync(join(projectRoot, 'scripts/cowork/check-ai-crms-runtime.mjs'), 'utf8')
const mediaResolver = readFileSync(join(root, 'main/agent/runtime/mediaRefResolver.ts'), 'utf8')
const mediaUpload = readFileSync(join(root, 'main/networking/api/mediaUpload.api.ts'), 'utf8')
...
const channelStore = readFileSync(join(root, 'renderer/control/src/store/channel.store.ts'), 'utf8')
const llmService = readFileSync(join(root, 'main/llm/coworkLlm.service.ts'), 'utf8')
const llmModels = readFileSync(join(root, 'main/llm/llmModels.ts'), 'utf8')
...
assert(aiCrmsRuntime.includes("createXpcMainEmitter<SessionApi>('CoworkSessionDao')"), 'AI-CRMS runtime should read the isolated bridged login session')
...
assert(packageJson.includes('"check:cowork": "node scripts/cowork/check-cowork.mjs"'), 'package scripts should expose the embedded Cowork parity suite')
...
assert(coworkWindow.includes('private coworkAgents = new Map<string, CoworkAgent>()'), 'Cowork should cache agents by chat session id')
assert(coworkWindow.includes('private hydratedCoworkAgentSessions = new Set<string>()'), 'Cowork should track which chat sessions have been hydrated')
assert(coworkWindow.includes('let agent = this.coworkAgents.get(key)'), 'Cowork should reuse cached chat agents')
assert(coworkWindow.includes('this.coworkAgents.set(key, agent)'), 'Cowork should cache new chat agents')
assert(coworkWindow.includes('includeConversationMemory = !this.hydratedCoworkAgentSessions.has(sessionKey)'), 'first turn should hydrate persisted chat memory once')
assert(coworkWindow.includes('const mediaInput = await this.buildAgentMediaInput(sessionKey, context?.attachedPaths)'), 'Cowork should build current-turn media refs from attached paths')
assert(coworkWindow.includes('resolveRuntimeMediaRefs({') && coworkWindow.includes('preferred transport:'), 'Cowork should resolve media transport per provider')
assert(coworkWindow.includes('uploadMediaRefsForProvider({') && coworkWindow.includes('mediaTransportForProvider(this.activeLlmProvider) === \'url\''), 'Cowork should try configured upload only for URL-first providers')
assert(coworkWindow.includes('mediaUploadSessionForProvider') && coworkWindow.includes("createXpcMainEmitter<SessionApi>('CoworkSessionDao')"), 'AI-CRMS media uploads should use the isolated bridged session for auth headers')
assert(coworkWindow.includes('media.push(ref)'), 'Cowork should pass local path media refs instead of inline payloads')
...
assert(!coworkWindow.includes('images.push({ data: readFileSync(path)'), 'Cowork should not pass base64 image payloads')
assert(coworkWindow.includes('async attachClipboardImage'), 'Cowork should materialize pasted screenshots in main')
assert(coworkWindow.includes("clipboard.readImage()"), 'pasted screenshots should be read from main clipboard without renderer bytes')
assert(coworkWindow.includes('onAgentSessionUsed: () => this.hydratedCoworkAgentSessions.add(sessionKey)'), 'successful turn should mark the agent session hydrated')
assert(coworkWindow.includes('freshSession: false') && coworkWindow.includes('media: turnMedia.media') && coworkWindow.includes('images: turnMedia.images'), 'normal chat turns should not force fresh sessions and should pass media refs')
...
assert(!coworkWindow.includes('resetAgentConversation'), 'main helper should not keep the removed reset conversation action')
assert(coworkWindow.includes('this.hydratedCoworkAgentSessions.delete(this.agentSessionKey(params?.sessionId))'), 'abort should clear hydration state')
...
assert(channelStore.includes('coworkSessionByTabId'), 'Control should keep one active Cowork session per operation tab')
