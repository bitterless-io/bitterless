...
const { CoachSettingsService, DEFAULT_START_URL, isDefaultStartUrl, normalizeUrl } = loadTsModule('@cowork-main/settings/coachSettings.service')
const settingsSource = readFileSync(join(root, 'main/settings/coachSettings.service.ts'), 'utf8')
const coworkWindowSource = readFileSync(join(root, 'main/windows/coworkWindow.helper.ts'), 'utf8')
const tabStoreSource = readFileSync(join(root, 'renderer/home/src/components/MenuBar/tab.store.ts'), 'utf8')
const startupDocs = readFileSync(join(workspaceRoot, 'docs/features/cowork-subapp.md'), 'utf8')
...
try {
  ...
  assert(coworkWindowSource.includes('if (!services.settings.hasCustomStartUrl()) return'), 'startup should not open a normal tab without a custom customer URL')
  assert(!coworkWindowSource.includes('services.settings.hasCustomStartUrl() ? settings.startUrl : await services.demo.start()'), 'startup should not default to local demo')
