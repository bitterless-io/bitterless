...
import { assert, assertCoworkAliasBoundary, assertNoStandaloneEntry, readCowork, readProject } from './_harness.mjs'
...
const handler = readProject('src/main/xpc/coworkWindow.handler.ts')
const appMain = readProject('src/main/app.main.ts')
const dataRoot = readCowork('main/data/coworkDataRoot.ts')
...
const prepareCli = readProject('scripts/prepare-cowork-cli.cjs')
const sqliteKey = readCowork('main/security/sqliteKey.service.ts')
const sqlitePreload = readCowork('preload/sqlite.preload.ts')
const coworkWindow = readCowork('main/windows/coworkWindow.helper.ts')
const windowHelper = readCowork('main/windows/window.helper.ts')
const workbenchStore = readCowork('renderer/workbench/src/workbench.store.ts')

assert(handler.includes('async openCoworkWindow()'), 'host must expose the Cowork open operation')
assert(handler.includes('current && !current.isDestroyed()') && handler.includes('coworkWindowHelper.show()'), 'repeat open must focus the existing Cowork instance')
assert(handler.includes("window.once('closed'") && handler.includes('destroyCoworkRuntime()'), 'Cowork close must clean its full runtime')
assert(handler.includes('destroyForHostQuit()') && appMain.includes('await coworkWindowHandler.destroyForHostQuit()'), 'host quit must await Cowork cleanup')
...
assert(appMain.includes("session.fromPartition(COWORK_PARTITION).protocol.handle"), 'E2E network isolation must cover the Cowork partition before opening the app')
...
assert(handler.includes("createXpcMainEmitter<SessionApi>('CoworkSessionDao')"), 'Cowork session storage must use an isolated channel')
assert(dataRoot.includes("join(app.getPath('userData'), 'cowork')"), 'Cowork files must live below Bitterless userData/cowork')
assert(dataRoot.includes("persist:bitterless-cowork"), 'Cowork web state must use an isolated persistent partition')
...
assert(!sqliteKey.includes('micromeet9527'), 'embedded Cowork must not retain the upstream fixed E2E key')
assert(sqlitePreload.includes("location.pathname.endsWith('/coworkSqlite/index.html')"), 'SQLite preload must not consume its bootstrap token in the initial about:blank document')
assert(coworkWindow.includes("process.env.BITTERLESS_E2E !== '1' && (is.dev || process.env.COACH_DEVTOOLS === '1')"), 'E2E mode must not open detached Cowork Control DevTools')
assert(windowHelper.includes("win.show()\n      if (process.platform === 'darwin') {\n        app.focus({ steal: true })\n        win.moveTop()\n      }\n      win.focus()"), 'repeat Open must show Cowork, activate and raise it on macOS, then focus Cowork')
...
assert(vite.includes("'app.main': resolve('src/main/app.main.ts')") && !vite.includes("resolve('src/cowork/main/app.main.ts')"), 'build must keep one Electron main entry')
for (const entry of ['coworkHome', 'coworkControl', 'coworkWorkbench', 'coworkSqlite']) {
  assert(vite.includes(`${entry}: resolve(`), `renderer build must include ${entry}`)
}
assert(vite.includes("coworkCoach: resolve('src/cowork/preload/coach.preload.ts')") && vite.includes("coworkSqlite: resolve('src/cowork/preload/sqlite.preload.ts')"), 'build must include both Cowork preloads')
...
assert(apps.includes("id: 'cowork'") && apps.includes('action: openCowork'), 'Mini Apps must expose Cowork through its host action')
assert(builder.includes('from: build/cowork-tools') && builder.includes('to: cowork-tools'), 'packaging must include the Cowork CLI bundle')
...
const loadSqliteKeyHarness = ({ viteEnv, dataRoot, e2e = false, packaged = false, safeStorage, fsOverrides = {} }) => {
  ...
  try {
    ...
    const localRequire = (specifier) => {
      ...
      if (specifier === '@cowork-main/data/coworkDataRoot') return { coworkDataRoot: () => dataRoot }
