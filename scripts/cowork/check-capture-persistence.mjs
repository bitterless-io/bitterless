...
const { buildPersistedRawCaptureRecords } = loadTsModule('@cowork-main/capture/captureRecordPersistence')
...
assert(
  coworkWindowSource.includes('if (this.capturing) await this.discardActiveCaptureForRestart()'),
  'startCapture should restart a live capture instead of reusing the old trace'
)
assert(
  coworkWindowSource.includes('private async discardActiveCaptureForRestart()'),
  'capture restart cleanup helper should exist'
)
assert(
  /discardActiveCaptureForRestart\(\)[\s\S]*this\.traceEvents = \[\]/.test(coworkWindowSource),
  'capture restart should clear in-memory trace events'
)
assert(
  /startCapture\(params[\s\S]*await this\.clearCaptureRecordEdits\(\)[\s\S]*xpcMain\.broadcast\('coach\/capture-started'/.test(coworkWindowSource),
