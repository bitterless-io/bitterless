...
const service = readFileSync(join(root, 'main/files/artifactWriter.service.ts'), 'utf8')
const cowork = readFileSync(join(root, 'main/windows/coworkWindow.helper.ts'), 'utf8')
const prompt = readFileSync(join(root, 'main/agent/prompt/coworkSysPrompt.ts'), 'utf8')
const catalog = readFileSync(join(root, 'main/agent/hostToolCatalog.ts'), 'utf8')
const workspaceDocs = readFileSync(join(workspaceRoot, 'docs/features/cowork-subapp.md'), 'utf8')
...
assert(cowork.includes("name: 'create_artifact'"), 'Cowork should expose create_artifact')
assert(cowork.includes('this.toolCreateArtifact(sessionKey'), 'create_artifact tool should call main implementation')
assert(cowork.includes('userDataPath: coworkDataRoot()'), 'create_artifact should use the isolated Cowork data root')
assert(cowork.includes('this.recordAgentArtifact(artifact)'), 'create_artifact should record reply artifacts')
...
try {
  ...
  const { writeArtifactFromJson } = loadTsModule('@cowork-main/files/artifactWriter.service')
