...
const { extractVariablesFromMessage } = loadTsModule('@cowork-main/agent/naturalLanguageVariables')
...
if (!coworkWindowSource.includes("const seed = Object.keys(c.seed).length ? clipInline(JSON.stringify(c.seed), MAX_AGENT_SKILL_INLINE_CHARS) : 'none'")) {
  console.error('[check-natural-language-vars] failed')
...
for (const [needle, message] of skillIndexChecks) {
  if (!coworkWindowSource.includes(needle)) {
    console.error('[check-natural-language-vars] failed')
