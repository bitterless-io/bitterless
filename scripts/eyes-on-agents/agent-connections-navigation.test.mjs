...
const stubsPlugin = {
  ...
  setup(buildApi) {
    ...
    buildApi.onResolve({ filter: /ClaudeEnvironmentCard\.vue$/ }, () => ({
      path: 'claude-environment-card',
      namespace: 'eyes-agent-connections-test'
    }));
    buildApi.onLoad({ filter: /.*/, namespace: 'eyes-agent-connections-test' }, (args) => {
      ...
      if (args.path === 'claude-environment-card') {
        return {
          contents: `
              import { h } from 'vue';
              export default {
                name: 'ClaudeEnvironmentCardStub',
                setup: () => () => h('div', {
                  class: 'claude-environment-stub',
...
try {
  await test('source and styles preserve the master-detail visual contract', () => {
    ...
    assert.match(
      componentSource,
      /name="eyesOnAgents__connections__codexPanel"[\s\S]*?eyesOnAgents__connections__appServer[\s\S]*?eyesOnAgents__connections__boundary[\s\S]*?eyesOnAgents__connections__bridge[\s\S]*?name="eyesOnAgents__connections__claudePanel"[\s\S]*?<ClaudeObservationCard\s*\/>[\s\S]*?<ClaudeEnvironmentCard\s*\/>/
    );
    assert.match(
      componentSource,
    ...
    assert.match(
      componentSource,
      /const connectionSections = \[\s*'codex',\s*'claude',\s*\] as const satisfies readonly ConnectionSection\[\];/
    );
    assert.match(
      componentSource,
      /const getSectionProvider = \(section: ConnectionSection\): ConnectionProvider =>\s*section === 'codex' \? 'codex' : 'claude';/,
      'the provider rail maps each section to its provider logo'
    );
    assert.doesNotMatch(
      componentSource,
      /providers\/claude-iterm2|iterm2\.png/,
      'no retired terminal-specific provider asset is present'
    );
    assert.doesNotMatch(componentSource, /claude-iterm2|ClaudeIterm2|claudeIterm2/);
    for (const action of [
      'connectAppServer',
      'disconnectAppServer',
      'syncThreads',
  ...
  await test('real DOM navigation supports click, keyboard, Off state, and state retention', async () => {
    ...
    try {
      ...
      assert.equal(tabs.length, 2, 'the rail shows exactly Codex and Claude');
      assert.ok(codexTab && claudeTab);
      assert.ok(codexPanel && claudePanel);
      ...
      assert.ok(claudePanel.querySelector('.claude-environment-stub'));
      assert.equal(
        claudePanel.querySelector('.claude-environment-stub')?.getAttribute('data-provider-enabled'),
        'false',
        'environment management remains under Claude while support is Off'
      );
      ...
      assert.equal(document.activeElement, claudeTab, 'ArrowUp wraps from Codex to Claude');
      ...
      assert.equal(codexTab.getAttribute('tabindex'), '-1');
      assert.equal(claudeTab.getAttribute('tabindex'), '0',
        'the roving tabindex keeps exactly one tab reachable across two sections');
