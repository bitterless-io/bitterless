...
assert(publish.includes("run('node', ['scripts/prepare-maestro-cli.cjs', options.platform])"), 'release builds should prepare the CLI before packaging')

assert(cliService.includes('process.resourcesPath') && cliService.includes("'maestro-tools'"), 'runtime should resolve the packaged CLI resource')
