...
assert(menuBar.includes('i18nHelper.menuBar.restartToUpdate'), 'Maestro ready update label should use the shared compact literal')
assert(menuBar.includes('i18nHelper.menuBar.downloadingUpdate'), 'Maestro downloading update label should use the shared compact literal')
assert(/updateStore\.downloading[\s\S]{0,120}downloadingUpdate[\s\S]{0,120}restartToUpdate/.test(menuBar), 'Maestro visible update label should name the state it is in')
assert(!/['"`](Downloading|Update|Updating)['"`]/.test(menuBar), 'Maestro update label must stay localized rather than hardcoded')
assert(menuBar.includes(':disabled="updateStore.downloading"'), 'Maestro update action should be disabled while downloading')
assert(menuBar.includes('downloadingVersion.replace(') && menuBar.includes('updateToVersion.replace('), 'Maestro update title should retain localized download and target-version detail')
