...
assert(maestroWindow.includes('private maestroAgents = new Map<string, MaestroAgent>()'), 'Maestro should cache agents by chat session id')
assert(maestroWindow.includes('private hydratedMaestroAgentSessions = new Set<string>()'), 'Maestro should track which chat sessions have been hydrated')
assert(maestroWindow.includes('let agent = this.maestroAgents.get(key)'), 'Maestro should reuse cached chat agents')
assert(maestroWindow.includes('this.maestroAgents.set(key, agent)'), 'Maestro should cache new chat agents')
assert(maestroWindow.includes('includeConversationMemory = !this.hydratedMaestroAgentSessions.has(sessionKey)'), 'first turn should hydrate persisted chat memory once')
assert(maestroWindow.includes('const mediaInput = await this.buildAgentMediaInput(sessionKey, context?.attachedPaths)'), 'Maestro should build current-turn media refs from attached paths')
assert(maestroWindow.includes('resolveRuntimeMediaRefs({') && maestroWindow.includes('preferred transport:'), 'Maestro should resolve media transport per provider')
assert(maestroWindow.includes('uploadMediaRefsForProvider({') && maestroWindow.includes('mediaTransportForProvider(this.activeLlmProvider) === \'url\''), 'Maestro should try configured upload only for URL-first providers')
assert(maestroWindow.includes('mediaUploadSessionForProvider') && maestroWindow.includes("createXpcMainEmitter<SessionApi>('MaestroSessionDao')"), 'AI-CRMS media uploads should use the isolated bridged session for auth headers')
assert(maestroWindow.includes('media.push(ref)'), 'Maestro should pass local path media refs instead of inline payloads')
...
assert(!maestroWindow.includes('images.push({ data: readFileSync(path)'), 'Maestro should not pass base64 image payloads')
assert(maestroWindow.includes('async attachClipboardImage'), 'Maestro should materialize pasted screenshots in main')
assert(maestroWindow.includes("clipboard.readImage()"), 'pasted screenshots should be read from main clipboard without renderer bytes')
assert(maestroWindow.includes('onAgentSessionUsed: () => this.hydratedMaestroAgentSessions.add(sessionKey)'), 'successful turn should mark the agent session hydrated')
assert(maestroWindow.includes('freshSession: false') && maestroWindow.includes('media: turnMedia.media') && maestroWindow.includes('images: turnMedia.images'), 'normal chat turns should not force fresh sessions and should pass media refs')
...
assert(maestroWindow.includes('this.hydratedMaestroAgentSessions.delete(this.agentSessionKey(params?.sessionId))'), 'abort should clear hydration state')
