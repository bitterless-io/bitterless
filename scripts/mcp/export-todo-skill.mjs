...
const HELP = `Export the portable Bitterless Todo skill for Codex and Claude Code.

Usage:
  yarn mcp:todo:skill:export [--output <zip-path>]

Options:
  --output <path>  Destination ZIP (default: dist/skills/bitterless-todo.zip)
...
const main = () => {
  try {
    ...
    console.log(`[todo-skill-export] wrote ${options.outputPath} (${fileCount} files)`);
    return 0;
  } catch (error) {
    const prefix = error instanceof InputError ? 'Input error' : 'FAIL';
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[todo-skill-export] ${prefix}: ${message}`);
