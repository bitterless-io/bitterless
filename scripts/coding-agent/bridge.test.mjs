...
try {
  ...
  assert.equal(upgradedService.getStatus('codex').configuration, 'configured');
...
`;
  ...
  assert.ok(JSON.stringify(installedClaude).includes('bitterless-claude-session-hook'));
  ...
  assert.ok(!removedClaudeText.includes('bitterless-claude-session-hook'));
  ...
  assert.equal(backgroundResult.surface, 'claude-code-background');
  assert.equal(backgroundResult.state, 'working');
  assert.equal(repositoryWrites, beforeBackgroundHookWrites);
  assert.equal(records.length, beforeBackgroundHookCount, 'hook must not create a duplicate CLI row');
  assert.equal(
    records.some((record) =>
  ...
  codexRecord.statusObservedAt = 2500;
