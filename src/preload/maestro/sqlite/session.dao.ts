...
import { SESSION_TOKEN_KEY, SESSION_WORKSPACE_KEY, SESSION_REGION_KEY } from '@maestro-shared/session.api'
...
// XpcPreloadHandler: instantiating it (bottom) auto-registers `xpc:MaestroSessionDao/<method>`,
// callable from main (createXpcMainEmitter) and any renderer (createXpcRendererEmitter)
// as <SessionApi>('MaestroSessionDao'). The token is the shell's single shared login session.
export class MaestroSessionDao extends XpcPreloadHandler implements SessionApi {
...
export const sessionDao = new MaestroSessionDao()
