...
import { buildPersistedRawCaptureRecords } from '@maestro-main/capture/captureRecordPersistence'
...
import { HostToolRegistry, type HostToolConfirmRequest } from '@maestro-main/agent/runtime/hostToolRegistry'
import type { AgentRuntimeImage, AgentRuntimeMediaRef } from '@maestro-main/agent/runtime/agentRuntime.types'
import { mediaTransportForProvider, resolveRuntimeMediaRefs } from '@maestro-main/agent/runtime/mediaRefResolver'
...
import { classifySkillApiCall, isMutatingHttpMethod, normalizeHttpMethod, type SkillApiSafetyDecision } from '@maestro-main/drive/apiSafety'
...
import { CoachSettingsService, normalizeUrl } from '@maestro-main/settings/coachSettings.service'
...
import { HOST_APPROVAL_HISTORY_KEY, HOST_TOOL_CONFIG_DOMAIN, HOST_TOOL_POLICY_KEY, WORKSPACE_CONFIG_DOMAIN, WORKSPACE_DEFAULT_KEY, type ConfigApi } from '@maestro-shared/config.api'
...
import type { CaptureMode, HeaderMap, NetworkTiming, TraceEvent } from '@maestro-shared/trace.types'
...
@injectable()
class MaestroWindowHelper extends WindowHelper implements MaestroLlmServiceState {
  ...
  private maestroAgents = new Map<string, MaestroAgent>()
  private trainerAgents = new Map<string, CoachAgent>()
  private delegateAgents = new Map<string, DelegateAgent>()
  // Tracks Maestro pi sessions that have already been hydrated with persisted chat memory.
  ...
  private hydratedMaestroAgentSessions = new Set<string>()
  ...
  create(): BrowserWindow {
    ...
    const controlLoad = is.dev && process.env['ELECTRON_RENDERER_URL']
      ? controlView.webContents.loadURL(`${process.env['ELECTRON_RENDERER_URL']}/maestro/control/index.html`)
      : controlView.webContents.loadFile(join(__dirname, '../renderer/maestro/control/index.html'))
  ...
  private createWorkbenchView(): Promise<void> {
    if (!this.browserWindow) return Promise.reject(new Error('Maestro window is not available.'))
  ...
  // Maestro `ingest_recording` tool: turn the CURRENT capture (renderer-edited records if present,
  ...
  applyLlmTarget(provider: string, model: string, effort: LlmEffort = 'low'): void {
    ...
    for (const agent of this.maestroAgents.values()) agent.setTarget(provider, model, effort)
  ...
  private onCapturedEvent(e: TraceEvent, tabId: string): void {
    ...
    // A generated skill carries a UI flow + an API fallback; Maestro prefers the UI loop.
    this.emit(e)
  }
...
export const maestroWindowHelper = iocHelper.bind({
  controller: MaestroWindowHelper,
  services: [MaestroLlmService]
}) as MaestroWindowHelper
...
function buildAgentTurnPrompt(params: {
  ...
  // The stable execution discipline lives in MaestroAgent.systemPrompt (sent once per session).
