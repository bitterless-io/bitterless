...
const resolveCliInvocation = (): CliInvocation => {
  ...
  return {
    command: bundled,
    argsPrefix: [],
    missing: `micromeet CLI not found. Expected packaged binary at ${bundled} or dev build at ${devDist}. Run yarn workspace @micromeet/cli build or yarn prepare:maestro-cli.`
  }
}
...
export const runAiCrmsReportReadiness = async (
  ...
  notes.push('AI-CRMS report-readiness runner uses the bundled micromeet CLI and the Maestro-synced encrypted credential.')
...
export const runAiCrmsMigration = async (
  ...
  notes.push('Migration auth uses MICROMEET_MIGRATION_TOKEN or --migration-token; Maestro does not store the token on the target.')
