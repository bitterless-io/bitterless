...
export function groupWorkflowTasks(runs: readonly WorkflowRunSnapshot[]): WorkflowTaskGroup[] {
  const categories: WorkflowTaskCategory[] = ['active', 'completed', 'failed', 'stopped']
  ...
  for (const run of newestWorkflowRuns(runs)) {
    ...
    for (const task of run.agents) byCategory[isWorkflowAgentLive(task.status) ? 'active' : task.status as 'completed' | 'failed' | 'stopped'].tasks.push(task)
  }
