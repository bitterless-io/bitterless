...
const installShortcutsForWebContents = (contents: WebContents, actions: ShortcutActions): void => {
  if (contents.session !== session.fromPartition(COWORK_PARTITION) || shortcutContents.has(contents)) return
...
// Bitterless retains the application menu and Cmd/Ctrl+Q. Only WebContents in Cowork's
