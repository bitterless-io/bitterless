...
export abstract class WindowHelper {
  ...
  /** preload bundle name under out/preload, e.g. 'coworkCoach.js' */
  protected abstract preloadFile: string
  /** renderer html under out/renderer, e.g. 'coworkHome/index.html' */
  ...
  private static getStateStore(): WindowStateStore {
    if (!WindowHelper.stateStore) WindowHelper.stateStore = new WindowStateStore(coworkDataRoot())
    return WindowHelper.stateStore
  }

  create(): BrowserWindow {
    // Cowork uses in-window chrome. Bitterless retains ownership of the application menu.
