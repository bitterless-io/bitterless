...
import type { HeaderMap, NetworkTiming, ResponseBodyOmittedReason, TraceEvent, UiActionStep, UiActionTarget } from '@cowork-shared/trace.types'
...
export class DebuggerCapture {
   ...
   * override script lives on it. `keepRuntime` is set on the pinned ai-crms tab, where authBridge
   * keeps Runtime enabled for its login-token bridge.
   */
    ...
    try {
      if (method === 'Fetch.requestPaused') {
        ...
        try {
          ...
          // the trace is persisted / ingested (coworkWindow.helper.emit).
