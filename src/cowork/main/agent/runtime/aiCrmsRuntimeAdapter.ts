...
import type { SessionApi, AuthSession } from '@cowork-shared/session.api'
import { normalizeCoachRegion } from '@cowork-shared/networking/coachRegion'
...
export class AiCrmsRuntimeAdapter implements AgentRuntimeAdapter {
  ...
  async createSession(options: AgentRuntimeSessionOptions): Promise<AgentRuntimeSession> {
    ...
    if (!session?.jwt_token) {
      throw new Error('Not signed in to AI-CRMS. Open the AI-CRMS tab and log in so Cowork can reuse the shared session token.')
    }
