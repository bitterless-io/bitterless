...
import { CommonService } from '@cowork-shared/iocHelper/ioc.helper'
import type { CoachSettings } from '@cowork-shared/coach.api'
import type { AuthSession, SessionApi } from '@cowork-shared/session.api'
import { AUTH_BROADCAST } from '@cowork-shared/session.api'
import { writeMicromeetCliCredential } from '@cowork-main/cli/micromeetCli.service'
import type { ConfigApi } from '@cowork-shared/config.api'
import { LLM_COMPRESSION_REMAINING_KEY, LLM_CONFIG_DOMAIN, LLM_TARGET_KEY } from '@cowork-shared/config.api'
import type { TraceEvent } from '@cowork-shared/trace.types'
import type { LlmConfig, LlmEffort, LlmProviderState } from '@cowork-shared/coach.api'
import {
...
import { coworkAuthPath, coworkModelsPath } from './llmPaths'
import { buildAiCrmsPiProviderConfig } from '@cowork-main/networking/api/aiCrmsRelay.api'
...
export interface CoworkLlmServiceState {
...
@injectable()
export class CoworkLlmService extends CommonService<CoworkLlmServiceState> {
  ...
  private readPiModelsJson(): Record<string, unknown> {
    try {
      const parsed = JSON.parse(readFileSync(coworkModelsPath(), 'utf8')) as unknown
  ...
  private async syncAiCrmsProviderModels(session?: AuthSession | null): Promise<boolean> {
    ...
    try {
      ...
      mkdirSync(dirname(coworkModelsPath()), { recursive: true })
      writeFileSync(coworkModelsPath(), JSON.stringify(doc, null, 2), 'utf8')
  ...
  private async readStoredLlmTarget(): Promise<LlmStoredTarget> {
    const fallback = this._state.readCoworkSettings()
  ...
  private async writeStoredLlmTarget(target: LlmStoredTarget): Promise<void> {
    ...
    this._state.saveCoworkSettings({ llmProvider: target.provider, llmModel: target.model, llmEffort: target.effort })
  }
  ...
  private async checkLlmProviderReady(provider: string, model: string): Promise<boolean> {
    ...
    try {
      ...
      const modelRegistry = pi.ModelRegistry.create(pi.AuthStorage.create(coworkAuthPath()), coworkModelsPath())
  ...
  private async performLlmLogin(provider: string, requestedMethod?: string): Promise<LlmConfig> {
    ...
    try {
      const pi = await loadPiAuthModule()
      mkdirSync(dirname(coworkAuthPath()), { recursive: true })
      const auth = pi.AuthStorage.create(coworkAuthPath())
  ...
  async logoutLlm(params?: { provider?: string }): Promise<LlmConfig> {
    ...
    try {
      const pi = await loadPiAuthModule()
      pi.AuthStorage.create(coworkAuthPath()).logout(provider)
    } catch (err) {
