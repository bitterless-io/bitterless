...
test('boot migrates product-home default_workspace to work, preserving files and never merging', t => {
  const home = temporary(t), userData = temporary(t)
  const { appDataRoot, appDataDir, ensureAppData, ensureWorkDirectory } = load(home, userData)
  const legacy = join(appDataRoot(), 'default_workspace')
  mkdirSync(legacy, { recursive: true })
  writeFileSync(join(legacy, 'notes.md'), 'existing work')
  ensureAppData()
  assert.equal(appDataDir('workspace'), join(appDataRoot(), 'work'))
  assert.equal(readFileSync(join(appDataDir('workspace'), 'notes.md'), 'utf8'), 'existing work')
  assert.equal(existsSync(legacy), false)
  mkdirSync(legacy)
  writeFileSync(join(legacy, 'notes.md'), 'older duplicate')
  assert.equal(ensureWorkDirectory(), appDataDir('workspace'))
  assert.equal(readFileSync(join(legacy, 'notes.md'), 'utf8'), 'older duplicate')
  assert.equal(readFileSync(join(appDataDir('workspace'), 'notes.md'), 'utf8'), 'existing work')
})
