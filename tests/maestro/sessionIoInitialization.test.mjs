...
test('new chat and immediate copy await the same disk write, with initial actual system/model metadata and no model request', async t => {
  ...
  assert.ok((await fs.readFile(join(a.path, 'README.md'), 'utf8')).includes('review-session.cjs'))
  assert.equal(JSON.parse(await fs.readFile(join(a.path, 'session-index.json'), 'utf8')).metadata.sessionId, 'new')
...
test('copied review path indexes all retained same-chat rotations, without raw rewrites or new model calls', async t => {
  const h = await harness(t)
  const first = await h.owner.ensureSessionIo({ sessionId: 'rotated' })
  await h.real.runInAgentSession('rotated', () => h.real.modelIoLog.append({ kind: 'prompt', name: 'fixture/model', subject: 'workflow', turn: 1, text: 'full child context', detail: { source: 'workflow', sessionId: 'rotated', runId: 'run-one', agentId: '1', attemptId: '1:1', turnId: 'turn-one' } }))
  const before = await fs.readFile(join(first.path, 'part-001.jsonl'), 'utf8')
  await new Promise(done => setTimeout(done, 3))
  await h.real.runInAgentSession('rotated', () => h.real.modelIoLog.openSession('reset'))
  await h.owner.ensureSessionIo({ sessionId: 'unrelated' })
  const copied = await h.owner.ensureSessionIo({ sessionId: 'rotated' }, 'missing-history')
  assert.equal(copied.ok, true); assert.notEqual(copied.path, first.path)
  const index = JSON.parse(await fs.readFile(join(copied.path, 'session-index.json'), 'utf8'))
  assert.deepEqual(index.metadata.directories, await Promise.all([first.path, copied.path].map(path => fs.realpath(path))))
  assert.equal(index.runs[0].runId, 'run-one'); assert.equal(index.runs[0].agents[0].agentId, '1')
  assert.equal(index.runs[0].records[0].file, await fs.realpath(join(first.path, 'part-001.jsonl')))
  assert.equal(await fs.readFile(join(first.path, 'part-001.jsonl'), 'utf8'), before)
  assert.deepEqual(h.counts(), { requests: 0, created: 0, resets: 0 })
})
