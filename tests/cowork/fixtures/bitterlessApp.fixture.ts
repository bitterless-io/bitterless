...
const pageMatches = (page: Page, rendererName: RendererName): boolean =>
  new RegExp(`/${rendererName}/index\\.html(?:$|[?#])`).test(page.url())
...
const attachRendererDiagnostics = (page: Page, errors: string[]): void => {
  ...
  page.on('console', (message) => {
    ...
    // host/Cowork error visible and ignore only that exact optional-host absence.
    ...
    if (!isExpectedUnpackagedQdrantAbsence) errors.push(`[console ${url}] ${text}`)
  })
}
...
export const test = base.extend<CoworkFixtures>({
  bitterless: async ({}, use) => {
    ...
    const tempRoot = mkdtempSync(join(tmpdir(), 'bitterless-cowork-e2e-'))
